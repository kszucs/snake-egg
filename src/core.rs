...
use crate::lang::{PythonAnalysis, PythonNode};
...
impl PyRewrite {
    #[new]
    #[args(name = "\"\"")]
    fn new(searcher: PyPattern, applier: PyPattern, name: &str) -> Self {
        let rewrite = Rewrite::new(name, searcher.pattern, applier.pattern)
            .expect("Failed to create rewrite");
        PyRewrite { rewrite }
    }
